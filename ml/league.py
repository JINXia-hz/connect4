...
6. The best round by original-ruler win rate is copied to ml/model_best.pt
   IMMEDIATELY, and every subsequent round generates its self-play games
   with the best checkpoint so far (never a degraded latest model).
...
BEST = os.path.join('ml', 'model_best.pt')
...
def main():
    ...
    for k in range(1, args.rounds + 1):
        ...
        # --- 0. Generate with the BEST checkpoint, not the latest ---
        # League v1 bug: self-play games were generated by whatever model.pt
        # held, so one degraded round poisoned the data pool and spiraled.
        if k > 1 and os.path.exists(BEST):
            shutil.copyfile(BEST, MODEL)
        ...
        # --- 2. Train + checkpoint ---
        t0 = time.time()
        # Train from scratch each round on ALL accumulated data. Warm-starting
        # (--init) was tried and abandoned: with a nearly-static dataset it
        # memorized positions (saturated outputs) and playing strength
        # collapsed within a few rounds.
        rc = subprocess.call([sys.executable, os.path.join('ml', 'train.py'),
        ...
        if r_o > best_rate:
            ...
            shutil.copyfile(MODEL, BEST)  # immediately usable next round
